function updateDashboard() {
  ...
  let interviewJobs = companyName.filter(
    (interview) => interview.status === "INTERVIEW",
  ).length;
  let rejectedJobs = companyName.filter(
  ...
  if (currentStatus) {
    currentStatus.innerText = document.querySelector(".empty")
      ? 0
      : document.getElementById("job-card-container").children.length;
  }
}
